...
router.get('/', authenticate, authorize('admin', 'manager'), async (req: Request, res: Response) => {
  ...
  try {
    const [
      ...
      db.query(`SELECT COUNT(*) as count, COALESCE(SUM(price),0) as revenue FROM bookings WHERE DATE(start_time)=$1 AND status IN ('confirmed','completed')`, [today]),
      db.query(`SELECT COUNT(*) as count FROM bookings WHERE DATE(start_time)=$1 AND status='confirmed'`, [today]),
      db.query(`SELECT COALESCE(SUM(price),0) as revenue, COUNT(*) as bookings FROM bookings WHERE start_time>=$1 AND status='completed'`, [monthStart]),
      db.query(`SELECT COUNT(DISTINCT customer_id) as count FROM bookings`),
      db.query(`
